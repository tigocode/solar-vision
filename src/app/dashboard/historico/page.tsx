        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-64">
            ...
            >
              ...
              <option value="Aguardando Upload">Status: Aguardando Upload</option>
            </select>
          </div>
              ...
              {filteredRecords.map((record, index) => (
                <div key={record.id} className="relative flex items-center justify-center animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: `${index * 150}ms`, fillMode: 'both' }}>
                  ...
                  <div className={`absolute left-[-31px] md:left-[-47px] mt-1.5 w-4 h-4 rounded-full border-4 border-white shadow-sm ring-2 ring-slate-200 ${record.status === 'Concluída' ? 'bg-indigo-500' : 'bg-amber-500 animate-pulse'}`}
                    style={{ backgroundColor: record.status === 'Concluída' ? brand.primaryColor : '#f59e0b', borderColor: '#fff' }}
                  />
                  {/* CARD DA INSPEÇÃO */}
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                      ...
                      <div className="space-y-4 flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          <span className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 ${record.status === 'Concluída' ? 'bg-indigo-50 text-indigo-700' : record.status === 'Em Processamento' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                            {record.status === 'Concluída' ? <BadgeCheck size={14} /> : <Activity size={14} className={record.status === 'Em Processamento' ? 'animate-pulse' : ''} />}
                            {record.status}
                          </span>
